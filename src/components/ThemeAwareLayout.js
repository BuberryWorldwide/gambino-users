// src/app/components/ThemeAwareLayout.js - Client component for theme-aware styling
'use client';
...
export default function ThemeAwareLayout({ children }) {
  ...
  return (
    <div className={`min-h-screen relative transition-colors duration-300 ${
      ...
      {/* Your existing mobile-optimized background effects - now theme-aware */}
      ...
      {/* Simplified background shapes for mobile - theme-aware */}
      <div className="fixed inset-0 pointer-events-none z-0 opacity-30 md:opacity-60">
        {/* Mobile-optimized gradient orbs */}
        <div className={`absolute -top-20 -right-20 md:-top-40 md:-right-40 w-48 h-48 md:w-80 md:h-80 rounded-full blur-2xl md:blur-3xl ${
          isDark 
            ? 'bg-gradient-to-br from-yellow-500/6 md:from-yellow-500/8 to-amber-600/3 md:to-amber-600/4' 
            : 'bg-gradient-to-br from-yellow-600/4 md:from-yellow-600/6 to-amber-700/2 md:to-amber-700/3'
        }`}></div>
        <div className={`absolute -bottom-20 -left-20 md:-bottom-40 md:-left-40 w-40 h-40 md:w-72 md:h-72 rounded-full blur-2xl md:blur-3xl ${
          isDark 
            ? 'bg-gradient-to-tr from-amber-600/8 md:from-amber-600/10 to-yellow-500/3 md:to-yellow-500/4' 
            : 'bg-gradient-to-tr from-amber-700/6 md:from-amber-700/8 to-yellow-600/2 md:to-yellow-600/3'
        }`}></div>
        
        {/* Minimal grid overlay - much lighter on mobile */}
        <div className={`absolute inset-0 ${isDark ? 'opacity-[0.02] md:opacity-[0.08]' : 'opacity-[0.01] md:opacity-[0.04]'}`}>
          <div 
            className="absolute inset-0" 
            style={{
              backgroundImage: `radial-gradient(circle at 1px 1px, rgba(234, 179, 8, ${isDark ? '0.25' : '0.15'}) 1px, transparent 0)`,
        ...
        <div className={`hidden md:block absolute top-1/3 left-1/5 w-16 h-16 border rounded-lg rotate-45 animate-spin ${
          isDark ? 'border-yellow-500/8' : 'border-yellow-600/12'
